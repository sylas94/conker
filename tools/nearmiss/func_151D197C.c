...
s32 func_151D197C(Trail *arg0) {
    ...
    nodes = arg0->unk94;
    if (info->unk0 != NULL) {
        owner = info->unk0->unk28;
        info->unk1C = owner->unk14C;
    }
    ...
    if (arg0->unk2C >= 2) {
        ...
        while (j != arg0->unk2D) {
            ...
            if (total > info->unkC) {
                if (node->unkC != 0.0f) {
                    frac = (total - info->unkC) / node->unkC;
                    next = &nodes[j];
                    dx = next->unk0 - node->unk0;
                    dy = next->unk4 - node->unk4;
                    dz = next->unk8 - node->unk8;
                    next->unk0 -= dx * frac;
                    next->unk4 -= dy * frac;
                    next->unk8 -= dz * frac;
                    node->unkC *= 1.0f - frac;
    ...
    if (arg0->unk2C >= 2) {
        i = arg0->unk2E;
        remaining = total;
        widthScale = (info->unk14 * info->unk1C) * invTotal;
        alphaScale = info->unk18 * invTotal;
        do {
            i--;
            if (i < 0) {
                i = arg0->unk25 - 1;
            ...
            seg = &nodes[i];
            seg->unk1C = (info->unk10 * info->unk1C) + (remaining * widthScale);
            seg->unk14 = remaining * alphaScale;
            remaining -= seg->unkC;
        } while (i != arg0->unk2D);
    }

    if ((arg0->unk1E & 1) && (arg0->unk1E & 8)) {
        info->unk8 = arg0->unk1C * 6;
    }
    return 1;
}
