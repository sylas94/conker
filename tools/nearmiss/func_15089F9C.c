...
s32 func_15089F9C(s32 arg0) {
    ...
    offset = arg0 * 4;
    best = -1;
    states = (s32 *)(D_800D23B0 + 0xE64);
    count = *(s32 *)(D_800D23B0 + 0x10);
    if (*(s32 *)(D_800D23B0 + offset + 0x15C) == 0) {
        minRange = D_8009DA44;
        maxRange = D_8009DA48;
    } else {
        minRange = D_8009DA4C;
    ...
    activeMask = *(s32 *)(D_800D23B0 + 0x14);
    bestScore = D_8009DA50;
    startPhase = D_8009DA54;

    for (i = 0; i < count; i++) {
        bit = 1 << i;
        if ((states[i] != 3) && ((activeMask & bit) != 0) &&
            ((*(s32 *)(D_800D23B0 + offset + 0x16C0) & (bit << 16)) != 0)) {
            scale = D_8009DA58 / (maxRange - minRange);
            obj = &D_800CC2D0[i];
            if (obj != 0) {
                dx = D_800CC2D0[arg0].x_position - obj->x_position;
                dz = D_800CC2D0[arg0].z_position - obj->z_position;
            ...
            score = sinf(((dist - minRange) * scale) + startPhase) + 1.0f;
            if (score > bestScore) {
                bestScore = score;
                best = i;
            }
