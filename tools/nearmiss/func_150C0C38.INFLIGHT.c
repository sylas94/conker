/* func_150C0C38 (game_EDE60, 1376 bytes) -- IN-FLIGHT, NON-MATCHING, UNVERIFIED.
 *
 * Rescued from the working tree at the end of wave 61 and reverted out of conker/src/, because it
 * was live with a .text of 0x890 against golden's 0x8b0 -- 32 BYTES SHORT -- which shifts every
 * symbol the linker places after it and breaks the ROM gate. A short .text is fatal even when the
 * body looks right: that exact failure mode cost a previous wave a score-0 function.
 *
 * NOT verified, NOT scored by me, and NOT to be pasted back without:
 *   1. an inventory check (every func_ definition AND every #pragma name in the real TU must
 *      exist here -- a whole-TU candidate that omits one breaks the link), and
 *   2. a fresh score, then the full ROM gate.
 * Kept only so the modelling work is not lost.
 */
#include <ultra64.h>
#include "functions.h"
#include "variables.h"
