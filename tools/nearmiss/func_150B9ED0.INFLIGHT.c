/* func_150B9ED0 (game_E7380, 1164 bytes) -- IN-FLIGHT, NON-MATCHING, UNVERIFIED.
 *
 * Rescued from the working tree at the end of wave 61 and reverted out of conker/src/, because it
 * was left LIVE (pragma removed) by an agent that stopped before verifying. It contributed 72
 * differing bytes to the linked image at ROM offset 0xE7485 and was the sole cause of a red ROM
 * gate.
 *
 * NOT scored by me, NOT verified. Do not paste back without:
 *   1. an inventory check -- every func_ definition AND every #pragma name in the real TU must
 *      exist in the candidate (a whole-TU candidate that omits one breaks the link),
 *   2. a fresh score, and
 *   3. the full ROM gate.
