...
s32 func_1508B9BC(void) {
    ...
    count = *(s32 *)(D_800D23B0 + 0x10);
    self = *(s32 *)(D_800D23B0 + 4);
    flags = (s32 *)(D_800D23B0 + 0x39C);
    links = (s32 *)(D_800D23B0 + 0x5DC);
    costs = (s32 *)(D_800D23B0 + 0x9DC);
    targets = (s32 *)(D_800D23B0 + 0x11C);
    quality = 1;
    if (count >= 9) {
        quality = 4;
    }
    func_1508C5B8(0, quality);
    func_1508A1BC();

    rowOffset = self * 16;
    weights = (u8 *)(D_800D23B0 + rowOffset + 0x15B4);
    bestIdx = -1;
    best = 10000000;
    i = 0;
    if (count > 0) {
        unitScale = D_8009DA64;
        closeScale = D_8009DA60;
        do {
            if ((flags[i] > 0) && (i != self)) {
                cost = -1;
                j = i * 16;
                end = j + 16;
                while ((j < end) && (links[j] != -1)) {
                    if (links[j] == self) {
                        cost = costs[j];
                        j = end;
                    } else {
                ...
                if (cost != -1) {
                    scale = (f32)(u32)weights[i] * unitScale;
                    if (targets[self] == i) {
                        scale *= closeScale;
                    }
                    cost = (s32)((f32)cost * scale);
                    if (cost < best) {
                        best = cost;
                        bestIdx = i;
                    }
    ...
    *(s32 *)(D_800D23B0 + self * 4 + 0x49C) = bestIdx;
    if (bestIdx != -1) {
        obj = &D_800CC2D0[self];
        other = &D_800CC2D0[bestIdx];
        dx = obj->x_position - other->x_position;
        dz = obj->z_position - other->z_position;
        *(s32 *)(D_800D23B0 + self * 4 + 0x45C) = (s32)sqrtf((dx * dx) + (dz * dz));
    } else {
        *(s32 *)(D_800D23B0 + self * 4 + 0x45C) = best;
    }
    return 0;
}
