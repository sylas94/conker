...
extern TileInfo *D_800C5338[];
extern Gfx *func_150C3160(Gfx *, struct127 *);

Gfx *func_1502F9FC(Gfx *gfx, s32 arg1) {
    struct127 *obj;
    TileInfo *tex;
    s32 id;
    s32 texIdx;
    ...
    if (D_800C5338[id] != NULL) {
        for (i = 0; i != 2; i++) {
            tex = &D_800C5338[id][texIdx];
            uls = ((s16 *)D_800CC2D0[arg1].unk27A)[i] + 2;
            ult = ((s16 *)D_800CC2D0[arg1].unk27A)[i + 2] + 2;
            lrs = (tex->width * 4) - 2;
            lrt = (tex->height * 4) - 2;
            gDPSetTileSize(gfx++, 5 - i, uls, ult, lrs, lrt);
        }
    }
    return gfx;
}
