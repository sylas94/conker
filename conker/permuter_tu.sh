...
PY
    ...
    if [ "$ns" != "$h" ]; then live=1; live_desc="$desc"; break; fi
    inert=$((inert + 1))
   done
   if [ $live -eq 0 ]; then
    ...
    echo "    live perturbation -- $live_desc"
    local neg_score
    ...
    if [ -n "$neg_score" ] && [ -n "$base_score" ] && [ "$neg_score" -gt "$base_score" ]; then
      ...
      echo "    FAIL: the object changed but the score did not -- the harness is not scoring $FUNC"; rc=1
    fi
   fi
  fi
  rm -rf "$B"
