...
// NON-MATCHING: the logic below is believed correct and is byte-identical except
// for one delay slot -- IDO speculatively hoists the `lw` reload of arg0 from the
// case 1 block into the `beq` delay slot, where the original leaves a `nop` and
// fills the slot with the default path's `move v0,zero` instead.
//
